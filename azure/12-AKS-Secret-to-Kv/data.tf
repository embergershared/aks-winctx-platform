...
data "kubernetes_namespace" "cert_manager_ns" {
  metadata {
    name = var.k8s_cert_mgr_ns_name
  }
}

data "kubernetes_secret" "root_ca_cert_secret" {
  metadata {
    name      = var.k8s_rootcacert_secret_name
...
data "azurerm_storage_container" "poc_st_blob_container" {
